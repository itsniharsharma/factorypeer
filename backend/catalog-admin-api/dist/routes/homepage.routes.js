...
import { createHomepageBannerBodySchema, createHomepageSupportCardBodySchema, createHomepageTileBodySchema, homepageBannerListQuerySchema, homepageContentIdParamsSchema, homepageSupportCardListQuerySchema, homepageTileListQuerySchema, updateHomepageBannerBodySchema, updateHomepageSupportCardBodySchema, updateHomepageTileBodySchema, } from "../validation/homepage-content.js";
const PREFIX = "/admin/catalog/homepage";
export async function registerHomepageRoutes(app, services) {
    const { homepage } = services;
    app.get(`${PREFIX}/banners`, async (req) => {
        const q = parseQuery(homepageBannerListQuerySchema, req.query);
        return homepage.listBanners(writeContext(req), q.status);
    });
    app.get(`${PREFIX}/banners/:id`, async (req) => {
        const { id } = parseParams(homepageContentIdParamsSchema, req.params);
    ...
    app.get(`${PREFIX}/category-tiles`, async (req) => {
        const q = parseQuery(homepageTileListQuerySchema, req.query);
    ...
    app.get(`${PREFIX}/support-cards`, async (req) => {
        const q = parseQuery(homepageSupportCardListQuerySchema, req.query);
