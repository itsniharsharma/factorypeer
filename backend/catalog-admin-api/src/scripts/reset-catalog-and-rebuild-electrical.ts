...
async function listAllProducts(): Promise<AnyObj[]> {
  const out: AnyObj[] = [];
  let skip = 0;
  const limit = 200;
  for (;;) {
    const { data } = await api<AnyObj[]>("GET", `/products?limit=${limit}&skip=${skip}`);
...
async function countVariants(productIds: string[]): Promise<number> {
  let total = 0;
  for (const id of productIds) {
    const { data } = await api<AnyObj[]>("GET", `/products/${id}/variants?limit=200`);
    total += Array.isArray(data) ? data.length : 0;
  }
  return total;
}

function flattenCategoryIds(tree: AnyObj[]): string[] {
  const ids: string[] = [];
  const walk = (nodes: AnyObj[]) => {
    for (const n of nodes) {
      const id = String(n["_id"] ?? "");
      if (id) ids.push(id);
      const children = Array.isArray(n["children"]) ? (n["children"] as AnyObj[]) : [];
...
function postOrderCategoryIds(tree: AnyObj[]): string[] {
  const ids: string[] = [];
  const walk = (nodes: AnyObj[]) => {
    for (const n of nodes) {
      const children = Array.isArray(n["children"]) ? (n["children"] as AnyObj[]) : [];
...
async function auditSnapshot(tag: string) {
  const products = await listAllProducts();
  const productIds = products.map((p) => String(p["_id"] ?? "")).filter(Boolean);
  const variantCount = await countVariants(productIds);
  const categories = (await api<AnyObj[]>("GET", "/categories/tree")).data;
  const banners = (await api<AnyObj[]>("GET", "/homepage/banners")).data;
  const tiles = (await api<AnyObj[]>("GET", "/homepage/category-tiles")).data;
  const cards = (await api<AnyObj[]>("GET", "/homepage/support-cards")).data;
  const groups = (await api<AnyObj[]>("GET", "/navigation/link-groups")).data;
  const footers = (await api<AnyObj[]>("GET", "/navigation/footer-content")).data;
...
async function resetByApi() {
  ...
  for (const p of products) {
    ...
    const variants = (await api<AnyObj[]>("GET", `/products/${pid}/variants?limit=200`)).data;
  ...
  const tree = (await api<AnyObj[]>("GET", "/categories/tree")).data;
  const postOrder = postOrderCategoryIds(Array.isArray(tree) ? tree : []);

  for (const cid of postOrder) {
    const schema = (await api<AnyObj | null>("GET", `/taxonomy/${cid}/spec-schema`)).data;
    if (schema?._id) {
      const sid = String(schema._id);
      const rows = (await api<AnyObj[]>("GET", `/spec-schemas/${sid}/rows?limit=200`)).data;
      for (const r of (Array.isArray(rows) ? rows : []).reverse()) {
        await api("DELETE", `/spec-rows/${String(r["_id"])}`);
      }
      const cols = (await api<AnyObj[]>("GET", `/spec-schemas/${sid}/columns`)).data;
...
async function runElectricalRebuildScript() {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`electrical-vertical-admin-api.mjs failed with code ${code}`));
    });
...
async function ensureHomepageShowcase() {
  ...
  const [banners, tiles, cards] = await Promise.all([
    api<AnyObj[]>("GET", "/homepage/banners"),
    api<AnyObj[]>("GET", "/homepage/category-tiles"),
    api<AnyObj[]>("GET", "/homepage/support-cards"),
  ]);
  ...
  if (existingBanner?._id) await api("PATCH", `/homepage/banners/${String(existingBanner._id)}`, bannerBody);
  else await api("POST", "/homepage/banners", bannerBody);

  const electrical = (await api<AnyObj[]>("GET", "/categories/root/children")).data.find((x) => x.slug === "electrical");
  ...
  if (existingTile?._id) await api("PATCH", `/homepage/category-tiles/${String(existingTile._id)}`, tileBody);
  else await api("POST", "/homepage/category-tiles", tileBody);
  ...
  if (existingCard?._id) await api("PATCH", `/homepage/support-cards/${String(existingCard._id)}`, cardBody);
  else await api("POST", "/homepage/support-cards", cardBody);
}
...
async function runValidation() {
  ...
  const [treeRes, mcbProductsRes, bannersRes, tilesRes, cardsRes] = await Promise.all([
    api<AnyObj[]>("GET", "/categories/tree"),
    api<AnyObj[]>("GET", "/products?status=published&limit=20&categoryId=miniature-circuit-breakers").catch(
      async () => api<AnyObj[]>("GET", "/products?status=published&limit=100"),
    ),
    api<AnyObj[]>("GET", "/homepage/banners?status=published"),
    api<AnyObj[]>("GET", "/homepage/category-tiles?status=published"),
    api<AnyObj[]>("GET", "/homepage/support-cards?status=published"),
  ]);
  ...
  for (const pid of productIds) {
    const vs = (await api<AnyObj[]>("GET", `/products/${pid}/variants?status=published&limit=50`)).data;
...
async function verifyStorefrontEmptyState() {
  ...
  const [megaMenu, suggest, exact, recentlyViewed] = await Promise.all([
    megaMenuRes.json() as Promise<AnyObj>,
    suggestRes.json() as Promise<unknown>,
    exactRes.json() as Promise<unknown>,
    recentlyViewedRes.json() as Promise<unknown>,
  ]);

  const columns = Array.isArray(megaMenu["columns"]) ? (megaMenu["columns"] as unknown[]) : [];
  const previewLinks = Array.isArray(megaMenu["previewLinks"]) ? (megaMenu["previewLinks"] as unknown[]) : [];
  const utilityLinks = Array.isArray(megaMenu["utilityLinks"]) ? (megaMenu["utilityLinks"] as unknown[]) : [];
...
async function main() {
  ...
  if (!rebuildOnly) {
    ...
    await resetByApi();
    let dbSpecCleanup:
      | { deletedSpecSchemas: number; deletedSpecColumns: number; deletedSpecRows: number }
