...
import { connectMongo, disconnectMongo, type CatalogModels } from "../db/connection.js";
...
function isRemoteSourceUnreachable(err: unknown): boolean {
  const o = err as { http_code?: number; message?: string };
  if (o.http_code === 404) return true;
...
function httpsUploadVariants(url: string): string[] {
  const trimmed = url.trim();
  const out: string[] = [trimmed];
  try {
    const u = new URL(trimmed);
    const bare = `${u.origin}${u.pathname}`;
    if (bare !== trimmed) out.push(bare);
  } catch {
 ...
 */
async function uploadHttpsToCloudinary(remoteUrl: string, folder: string, opts?: { label?: string }) {
  ...
  for (let i = 0; i < variants.length; i++) {
    ...
    try {
      ...
      if (!isRemoteSourceUnreachable(e)) throw e;
      lastUnreachable = e;
      if (i < variants.length - 1) {
        console.warn(
          `[migrate] remote 404 for ${opts?.label ?? "asset"} — retrying without query (${remoteUrl.slice(0, 88)}…)`,
        );
        continue;
      }
    }
  ...
  console.warn(
    `[migrate] remote still unreachable (${remoteUrl.slice(0, 96)}…) — uploading fallback seed: ${fallback}`,
  );
  try {
    ...
    throw new Error(
      `Migration could not fetch ${remoteUrl} and fallback upload failed (${fallback}): ${String(lastUnreachable)} | ${String(fallbackErr)}`,
    );
  }
}

async function uploadToCloudinary(sourceUrl: string, folder: string, opts?: { label?: string }) {
...
async function migrateProducts(models: CatalogModels) {
...
async function migrateHomepage(models: CatalogModels, modelName: keyof CatalogModels, folder: string) {
  const Model = models[modelName] as CatalogModels["HomepagePromoBanner"];
  const docs = await Model.find({});
  let n = 0;
  for (const doc of docs) {
    const o = doc.toObject() as Record<string, unknown>;
    const image = o["image"] as { url?: string } | undefined;
    const legacyUrl = o["imageUrl"] as string | undefined;
    const url = image?.url ?? legacyUrl;
    if (!url?.trim()) continue;
    if (isCloudinaryDeliveryUrl(url)) continue;
    try {
      const res = await uploadToCloudinary(url, folder, {
        label: `${String(modelName)} ${String(doc._id)}`,
      });
      const alt = (o["imageAlt"] as string | undefined) ?? (image as { alt?: string } | undefined)?.alt;
