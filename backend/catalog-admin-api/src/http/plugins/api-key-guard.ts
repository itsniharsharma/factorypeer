 ...
 */
export function registerApiKeyGuard(app: FastifyInstance, adminApiKey: string | undefined) {
  if (!adminApiKey?.length) {
    app.log.warn(
      "CATALOG_ADMIN_API_KEY is not set — catalog-admin-api accepts unauthenticated requests. Set in production.",
    );
    return;
  }
