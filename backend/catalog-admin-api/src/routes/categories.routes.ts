...
import {
  attachSpecSchemaBodySchema,
  categoryIdParamsSchema,
  createCategoryBodySchema,
  moveCategoryBodySchema,
  reorderSiblingsBodySchema,
  setCategoryKindBodySchema,
...
export async function registerCategoryRoutes(app: FastifyInstance, services: CatalogAdminServices) {
  const { categories } = services;

  app.get(`${PREFIX}/tree`, async (req) => categories.getTree(writeContext(req)));

  app.get(`${PREFIX}/root/children`, async (req) => {
    const q = parseQuery(categoryChildrenQuerySchema, req.query as Record<string, string>);
    return categories.listChildren(null, writeContext(req), { status: q.status });
  });

  app.get(`${PREFIX}/:id/children`, async (req) => {
    const { id } = parseParams(categoryIdParamsSchema, req.params as Record<string, string>);
    const q = parseQuery(categoryChildrenQuerySchema, req.query as Record<string, string>);
