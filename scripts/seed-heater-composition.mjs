...
async function api(method, suffix, opts = {}) {
  ...
  if (!res.ok) {
    log('[http] ERROR', res.status, res.statusText, json);
...
async function main() {
  ...
  try {
    ...
    log('✓ Found category:', categoryId, category.name ?? category.title ?? 'heater');
    ...
    try {
      const existingRes = await api('GET', `/compositions/category/${categoryId}`);
      if (existingRes.data) {
        compositionId = existingRes.data._id;
        log('✓ Composition already exists. ID:', compositionId);
        log('   Skipping creation, will verify rendering...');
      }
    } catch (err) {
      // Composition doesn't exist, will create it
      log('   No existing composition found, will create one');
    }
    ...
    if (!compositionId) {
      ...
      const payload = JSON.parse(JSON.stringify(compositionPayload, (key, value) => {
      ...
      try {
        ...
        compositionId = createRes.data?._id || createRes._id;
        log('✓ Composition created:', compositionId);
      } catch (err) {
        // Check if it's a conflict error (composition already exists)
        if (err.message.includes('409')) {
          log('   Composition already exists, fetching existing...');
          const existingRes = await api('GET', `/compositions/category/${categoryId}`);
          compositionId = existingRes.data?._id;
          log('✓ Using existing composition:', compositionId);
        } else {
          throw err;
        }
      }
      ...
      const compositionPayload = {
        ...
        familySections: [
          // FAMILY 1: Portable Utility Heaters
          {
            id: 'portable-util',
          ...
          // FAMILY 2: Radiant Infrared Heaters
          {
            id: 'radiant-ir',
          ...
          // FAMILY 3: Warehouse Heating Systems
          {
            id: 'warehouse',
    ...
    // Step 4: Publish the composition
    log('\n4. Publishing composition...');
    const publishRes = await api('POST', `/compositions/${compositionId}/publish`, {
      json: true,
      body: '{}'
    });
    log('✓ Composition published');

    // Step 5: Verify via API
    log('\n5. Verifying composition via API...');
    const getRes = await api('GET', `/compositions/category/${categoryId}`);
    const published = getRes.data;
    
    if (published) {
      log('✓ Composition retrieved from API');
      log('  - Status:', published.status);
      log('  - Heading:', published.overviewSection?.heading);
      log('  - Family Sections:', published.familySections?.length);
      published.familySections?.forEach((fs, idx) => {
        log(`    [${idx}] ${fs.title} (${fs.table?.rows?.length || 0} rows)`);
      });
    }

    // Step 6: Create demo render test via HTTP endpoint
    log('\n6. Verifying category page endpoint...');
    const categoryPageUrl = `${getApiBase().replace('admin', 'catalog')}/categories/${categoryId}`;
    log('   Category endpoint:', categoryPageUrl);
    log('   (Storefront will render composition on: /category/electrical/home-stuff/heater)');
    ...
    log('\nStorefront Route: /category/electrical/home-stuff/heater');
